...
void matmul_forward_mpi(float* out, const float* inp, const float* weight, const float* bias,
                        int B, int T, int C, int OC, MPI_Comm comm) {
    ...
    int local_B = B / size;
    int rem = B % size;
    if (rank < rem) local_B++; // distribute remainder

    // Compute the offset and size for this rank
    int* batch_sizes = (int*)malloc(size * sizeof(int));
    int* displs = (int*)malloc(size * sizeof(int));
    for (int i = 0, b = 0; i < size; ++i) {
        batch_sizes[i] = B / size + (i < rem ? 1 : 0);
        displs[i] = b;
        b += batch_sizes[i];
    }

    int local_B_start = displs[rank];
    int local_TB = local_B * T;
    int local_inp_size = local_TB * C;
    int local_out_size = local_TB * OC;

    float* local_inp = (float*)malloc(local_inp_size * sizeof(float));
    float* local_out = (float*)malloc(local_out_size * sizeof(float));

    // Scatter input
    int* sendcounts_inp = (int*)malloc(size * sizeof(int));
    int* displs_inp = (int*)malloc(size * sizeof(int));
    for (int i = 0; i < size; ++i) {
        sendcounts_inp[i] = batch_sizes[i] * T * C;
        displs_inp[i] = displs[i] * T * C;
    }

    MPI_Scatterv(inp, sendcounts_inp, displs_inp, MPI_FLOAT,
                 local_inp, local_inp_size, MPI_FLOAT, 0, comm);

    // Broadcast weight and bias
    MPI_Bcast((void*)weight, OC * C, MPI_FLOAT, 0, comm);
    if (bias != NULL) {
        MPI_Bcast((void*)bias, OC, MPI_FLOAT, 0, comm);
    }
    ...
    for (int b = 0; b < local_B; b++) {
        for (int t = 0; t < T; t++) {
            ...
            for (int o = 0; o < OC; o++) {
                float val = (bias != NULL) ? bias[o] : 0.0f;
                for (int i = 0; i < C; i++) {
                    val += local_inp[bt * C + i] * weight[o * C + i];
                }
                local_out[bt * OC + o] = val;
            }
        }
    }

    // Gather output
    int* recvcounts_out = (int*)malloc(size * sizeof(int));
    int* displs_out = (int*)malloc(size * sizeof(int));
    for (int i = 0; i < size; ++i) {
        recvcounts_out[i] = batch_sizes[i] * T * OC;
        displs_out[i] = displs[i] * T * OC;
    }

    MPI_Gatherv(local_out, local_out_size, MPI_FLOAT,
                out, recvcounts_out, displs_out, MPI_FLOAT, 0, comm);
